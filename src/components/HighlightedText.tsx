...
export function HighlightedText(
    ...
    return <>
        {chunks.map(({text, isMatch}) => isMatch ? <mark>{text}</mark> : text )}
    </>
}
...
function* sliceChunks(text: string, query: string): Generator<Chunk, void, void> {
    ...
    for (const matchIndex of findAllMatches(text, query)) {
        ...
        const nonMatchBegin = previousMatchIndex === null ? 0 : previousMatchIndex + query.length
